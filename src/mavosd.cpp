...
void 
MavOSD:: show_text(char (*text)[15])
{
    memcpy(craftname, *text, sizeof(craftname));
}

void 
MavOSD:: check_system_status()
{
...
MavOSD:: blink_sats()
{
    if(general_counter % 900 == 0 && set_home == 1 && blink_sats_orig_pos > 2000){
      invert_pos(&_osd_gps_sats_pos, &blink_sats_blank_pos);
    }
    else if(set_home == 0){
      _osd_gps_sats_pos = blink_sats_orig_pos;
    }
}

void
MavOSD:: mAh_drawn_calc()
{
...
void
MavOSD:: display_wind_speed_and_direction()
{
...
void
MavOSD:: set_battery_cells_number()
{
...
void
MavOSD:: mavl_receive(mavlink_message_t* msg, mavlink_status_t* status)
{
    switch(msg->msgid)
    {
...
void
MavOSD::  GPS_calc_longitude_scaling(int32_t lat)
{
...
void
MavOSD:: GPS_distance_cm_bearing(int32_t *currentLat1, int32_t *currentLon1, int32_t *destinationLat2, int32_t *destinationLon2, uint32_t *dist, int32_t *bearing)
{
...
void
MavOSD::GPS_calculateDistanceAndDirectionToHome(void)
{
...
void
MavOSD::sendToOSD(mavlink_message_t* msg, mavlink_status_t* status)
{
    //receive mavlink data
    mavl_receive(msg, status);
